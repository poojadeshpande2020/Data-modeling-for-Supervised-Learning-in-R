...
final_df$TotalSqft <- final_df$FirstFlrSF + final_df$SecondFlrSF + final_df$TotalBsmtSF
...
#Q.1
ggplot(final_df,aes(x =GrLivArea,y = SalePrice))+geom_point(col="tomato1")+
  ggtitle("GrLivArea and SalePrice")+geom_smooth(method = 'lm')

model1 <- lm(SalePrice ~ GrLivArea,data = final_df)
summary(model1)
anova(model1)
...
plot(final_df$SalePrice,standardized_residual,col = 'red',main = 'Residual plot',
     xlab = 'GrLivArea',ylab = 'Standardized Residuals')
...
#Q.2
ggplot(final_df,aes(x =OverallQual,y = SalePrice))+geom_point(col="turquoise")+
  ggtitle("OverallQual and SalePrice")+geom_smooth(method = 'lm',col='tomato1')



model2 <- lm(SalePrice ~ OverallQual,data = final_df)
summary(model2)
anova(model2)
...
#Plotting standardized residuals
hist(standardized_residual, col = 'blue')
plot(final_df$OverallQual,standardized_residual,col = 'red',main = 'Residual plot',
     xlab = 'OverallQual',ylab = 'Standardized Residuals')
...
#Q.4
model3 <- lm(SalePrice ~ GrLivArea + OverallQual,data = final_df)
summary(model3)
anova(model3)
...
hist(standardized_residual, col = 'blue')
p1 <- plot(final_df$OverallQual,standardized_residual,col = 'red',main = 'Residual plot',
     xlab = 'OverallQual',ylab = 'Standardized Residuals')
p2 <- plot(final_df$GrLivArea,standardized_residual,col = 'red',main = 'Residual plot',
           xlab = 'OverallQual',ylab = 'Standardized Residuals')
...
#Q.5
model4 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea,data = final_df)
summary(model4)
anova(model4)

#Plotting standardized residuals
standardized_residual <- rstandard(model4)
hist(standardized_residual, col = 'blue')
...
#Q.6
#Model 1 with logSalePrice
ggplot(final_df,aes(x =GrLivArea,y = logSalePrice))+geom_point(col="tomato1")+
  ggtitle("GrLivArea and logSalePrice")+geom_smooth(method = 'lm')

model1 <- lm(logSalePrice ~ GrLivArea,data = final_df)
summary(model1)
anova(model1)

#Plotting standardized residuals
standardized_residual <- rstandard(model1)
hist(standardized_residual, col = 'blue')
...
#Model 3 with logSalePrice
model3 <- lm(logSalePrice ~ GrLivArea + OverallQual,data = final_df)
summary(model3)
anova(model3)
...
#Model 4 with logSalePrice
model4 <- lm(logSalePrice ~ GrLivArea + OverallQual + LotArea,data = final_df)
summary(model4)
anova(model4)
...
Regression(SalePrice ~ GrLivArea + OverallQual + LotArea,data = final_df)

new_df <- final_df[(final_df['SID']!=957),]
new_df <- new_df[(new_df['SID']!=2116),]

dim(new_df)

Regression(SalePrice ~ GrLivArea + OverallQual + LotArea,data = new_df)

new_model4 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea,data = new_df)
summary(new_model4)
residualPlots(new_model4)
influenceIndexPlot(new_model4)

##

new_df <- new_df[(new_df['SID']!=2072),]
...
model5 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea,data = final_df)
summary(model5)
anova(model5)

model6 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF,data = final_df)
summary(model6)
anova(model6)

model7 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + FirstFlrSF,data = final_df)
summary(model7)
anova(model7)

model8 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + SecondFlrSF,data = final_df)
summary(model8)
anova(model8)

model9 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + GarageArea,data = final_df)
summary(model9)
anova(model9)

model10 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + GarageArea + WoodDeckSF,data = final_df)
summary(model10)
anova(model10)

model11 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + GarageArea + 
               OpenPorchSF,data = final_df)
summary(model11)
anova(model11)

model12 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + GarageArea
             +PoolArea,data = final_df)
summary(model12)
anova(model12)

model13 <- lm(SalePrice ~ GrLivArea + OverallQual + LotArea + MasVnrArea + TotalBsmtSF + GarageArea
             + age,data = final_df)
summary(model13)
anova(model13)
standardized_residual <- rstandard(model13)
hist(standardized_residual,col="blue")
plot(model13)
influenceIndexPlot(model13)
